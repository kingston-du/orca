...
Implementation is underway. **Phase 0 — Expo engineering baseline** is complete: the app runs in the iOS Simulator and the Home / Camera / Memories shell exists. The private GitHub repository is connected as `origin`; local `main` tracks `origin/main` at pushed runtime commit `aa2d4b5`, and `.env` is not tracked. The toolchain is pinned to Node `24.14.1` and npm `11.11.0`. Expo is explicitly iOS/Android-only, iPad support is disabled, and direct web configuration/script/dependencies are removed. Expo compatibility, TypeScript, and all 20 `expo-doctor` checks pass; the current native-only changes are not yet committed. The pushed Auth WIP still uses raw AsyncStorage, has an incomplete sign-in screen, and lacks a protected session gate; lint has eight WIP warnings and formatting fails on that draft. The production-oriented audit has 11 moderate and zero high/critical findings, all in Expo configuration/build transitives; do not force-fix or install `uuid` directly. A hosted Supabase development project exists, but there are no local migrations, generated database types, database tests, CI, production project, or EAS setup.

The active milestone is **Phase 1 — Recoverable engineering and backend foundation**. Commit the native-only changes as `chore: remove web support`; the next implementation task is the audited direct-dependency cleanup/exact Supabase pin in `PROJECT.md`. Do not remove navigation peers/assets, skip to Auth UI, or force-fix the audit output.
