...
- Confirm whether the active checkpoint is approved. Phases 1 and 2 are complete and promoted on `codex/friend-first-rebaseline`: local and hosted development run the same six-migration history, and hosted carries the private `avatars` bucket, the `finalize-avatar` and `reconcile-operations` Edge Functions, both Vault secrets, `pg_cron`/`pg_net`, and both Cron schedules, all verified. Phase 3 is not authorized until the founder gives the precise approval in Section 31. Physical-iPhone acceptance for 1B and 2C remains open, and production resources still do not exist.
