...
Implementation is underway. **Phase 0 — Expo engineering baseline** is complete: the app runs in the iOS Simulator and the Home / Camera / Memories / Settings shell exists. The private GitHub repository is connected as `origin`, and `.env` is not tracked. The toolchain is pinned to Node `24.14.1` and npm `11.11.0`; Supabase CLI `2.109.1` and Expo Doctor `1.20.1` remain exactly pinned. The local Supabase stack runs Postgres `17.6` and cleanly replays all eight current migrations. The first six are applied to hosted development with matching history, all 157 remote assertions passing, and clean hosted Security/Performance Advisors. Two local-only migrations add the hash-only invitation/member lifecycle plus deterministic account-deletion succession and the pre-media Circle deletion contract. The app now has a Circle hub and create/join/detail flows with TanStack Query ownership, full member/invite administration, explicit destructive confirmations, exact exited/deleted cache removal, and ephemeral raw invite tokens. A clean reset, all 247 local assertions, database lint, exact generated types, all 50 app tests, an iOS export, and Expo Doctor 20/20 pass. The exposed onboarding and Circle RPCs are security invoker and delegate only narrow atomic writes to independently validating helpers in the unexposed `private` schema. The temporary legal text remains founder-only. Numbered tutorials in `docs/course/` explain completed checkpoints in depth. The Supabase client retains encrypted session storage and one AppState refresh owner; Auth state remains outside TanStack Query and identity transitions clear all current user-scoped data. EAS project `@kingstondu/orca` (`dc295559-844a-48f4-924a-e653c31602cd`) and Android build `38ec9b82-8472-4e99-9b04-fa8bbdb1b013` remain configured; physical acceptance is deferred. Apple Developer enrollment/physical iPhone work remains deferred but required before external iOS testing. GitHub currently reports one high `brace-expansion` denial-of-service alert through Jest/glob tooling and one moderate `uuid` alert through Expo's `xcode` build tooling; neither path is app-runtime code, and no Expo-compatible nonbreaking fix is available. Do not force-fix, downgrade the Expo toolchain, or install `uuid` directly. There is no production Supabase project.

The active implementation milestone is **Phase 3 — Circles, membership, and invitations**. Its database foundation, local invite/member lifecycle, account-succession/deletion contract, and simulator app flow are verified. The linked CLI ref and ignored app URL both identify healthy `orca-dev` (`evuqnmvcnqhkzkitszqp`), and a fresh dry run names exactly migrations `20260729000116` then `20260729064610`; it applied nothing. Promotion is the next separate explicit gate, followed by 247 remote assertions, matching history, and both hosted advisors. Invitation-gated production signup is the remaining Phase 3 product dependency. Phase 2's hosted Auth/SMTP/bot protection, final legal copy, remaining visual passes, and physical-device acceptance remain pre-external-tester gates. Do not amend applied migrations or make ad-hoc dashboard changes.
