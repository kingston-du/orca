...
Implementation is underway. **Phase 0 — Expo engineering baseline** is complete: the app runs in the iOS Simulator and the Home / Camera / Memories shell exists. The private GitHub repository is connected as `origin`; local `main` tracks `origin/main` through documentation commit `7deeb86`, which follows pushed security-foundation commit `b5ea078`, and `.env` is not tracked. The toolchain is pinned to Node `24.14.1` and npm `11.11.0`. Expo is explicitly iOS/Android-only, iPad support is disabled, direct web configuration/script/dependencies are removed, and verified production/development identities resolve separately through `app.config.js`. The direct dependency audit is complete: unused template packages are removed and current `@supabase/supabase-js` `2.110.8` plus `react-native-url-polyfill` `4.0.0` are exactly pinned. Supabase CLI `2.109.1` is exactly pinned. The Docker-backed local stack is running, resets from version-controlled inputs, and answers direct queries on Postgres `17.6`. The committed security-foundation migration establishes the `private` schema and least-privilege defaults; a clean reset applies it, all 16 pgTAP assertions pass, Security Advisor reports no warnings, and the probes leave no residue. Expo compatibility, TypeScript, and all 20 `expo-doctor` checks pass. The pushed Auth WIP still uses raw AsyncStorage, has an incomplete sign-in screen, and lacks a protected session gate; lint has eight WIP warnings and formatting fails on that draft. The production-oriented audit has 11 moderate and zero high/critical findings, all in Expo configuration/build transitives; do not force-fix or install `uuid` directly. A hosted Supabase development project exists, but there are no generated database types, CI, production project, or EAS setup.

The active milestone is **Phase 1 — Recoverable engineering and backend foundation**. The hosted-development linking and security-promotion checkpoint is complete and ready to commit. The pinned CLI is authenticated and linked only to the healthy hosted development project `orca-dev` (`evuqnmvcnqhkzkitszqp`, `ca-central-1`), whose ref matches the ignored app `.env`. Hosted development records both migrations `20260726040517` and `20260726233832`, matching local history. The guarded second migration revoked direct execution of the pre-existing Supabase `public.rls_auto_enable()` helper from `PUBLIC`, `anon`, `authenticated`, and `service_role`; hosted Security Advisor reports no warnings. Its post-push pg-delta catalog-cache warning was non-transactional: migration history and advisor state confirm the database change succeeded. Clean local reset succeeds, all 16 pgTAP assertions pass both locally and against hosted development, and a direct hosted query confirms every transaction probe is absent. The test uses transaction-local `set role postgres` because CLI 2.109.1's linked-test login may assume but does not inherit `postgres` schema privileges. Review and commit this checkpoint before starting the distinct generated-database-types and CI checkpoint. Do not amend either applied migration, make ad-hoc dashboard changes, or resume Auth UI yet.
