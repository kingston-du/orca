...
- Confirm whether the active checkpoint is approved. Phase 5 is complete and promoted on `codex/friend-first-rebaseline`: local and hosted development run the same **ten**-migration history. Hosted carries the private `avatars` and `moment-media` buckets, the `finalize-avatar`, `finalize-moment`, and `reconcile-operations` Edge Functions, both Vault secrets, `pg_cron`/`pg_net`, and both Cron schedules, all verified against real HTTP. Checkpoint 5B was approved together with one founder-directed product change — an author now sees their own posted Recent Moments on Home — and is promoted; only its physical-device pass is outstanding. 5A's own physical-device pass is complete (left=older accepted) and found two device-only defects, both fixed at `51ae53a`; the post-acceptance restyle means most of that pass must be re-run against the current geometry — see Section 27. Physical-iPhone acceptance for 1B's remaining items, 2C, Phase 3's capture-metadata fixtures, Phase 4's interruption/network publish cases, and 5B's 50-card runs remains open, and production resources still do not exist.
