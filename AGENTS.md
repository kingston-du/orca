...
- Confirm whether the active checkpoint is approved. Checkpoints 9A and 9B are **implemented and green on local development only**: local runs sixteen migrations, hosted still runs thirteen, and their schema/function promotions each require explicit approval. 9B's second-project local restore drill passes and destroys its synthetic resources, but no paid database backup, off-site archive, production backup secret/schedule, or remote drill resource exists; its ≤24-hour RPO/RTO values remain targets. Phase 8 is also local only; its promotion/deployment, Expo push access token, native rebuild, and two-device acceptance remain open. Phase 7 and the post-Phase-7 SQLSTATE correction are **implemented and promoted**: hosted carries the service-only `moderation-evidence` bucket, `moderate-report` is ACTIVE with JWT verification, and the Phase-7 secret-only `reconcile-operations` worker is ACTIVE. TOTP enrol and verify are enabled, and the real hosted acceptance drill proved AAL1 denial, AAL2 list/read/evidence/action flows, suspension/reinstatement, stale-command denial, direct private/Storage denial, and immediate operator revocation. Exact disposable cases, evidence objects, operators, and Auth users were removed; the append-only redacted audit actions remain by design. Phase 6 is complete and promoted on `codex/friend-first-rebaseline`, and Phase 5 is complete, promoted, and physically accepted. Two founder-directed product changes are settled and both say the same thing: an author sees their own posted Recent Moments on Home (Checkpoint 5B), and an author sees their own Moments in Highlights alongside their friends' (Phase 6). **Treat "a person's own content appears wherever everyone else's is shown" as the standing rule, not as two exceptions.** Phase 5 owes no device gate. Phase 6's physical-feel item is outstanding, and haptics are deferred by founder direction so a later native rebuild can carry several changes at once. Physical-iPhone acceptance for 1B's remaining items, 2C, Phase 3's capture-metadata fixtures, and Phase 4's interruption/network publish cases remains open, and production resources still do not exist. Phase 7's four safety decisions are **approved (2026-08-03)** and implemented as written in [the safety policy decisions](docs/operations/2026-08-02-safety-policy-decisions.md). Remaining founder-owned release operations are the real support mailbox/configuration, a permanent separate operator account with founder-held TOTP, and the Sentry project/DSN/native source-map verification; Checkpoint 9D still owns external-beta legal text. Do not invent any of these identities or credentials.

- Checkpoint 9B added one rule worth carrying forward: **a deletion tombstone outranks both an in-flight backup copy and an older database recovery point.** The cleanup-row trigger records backup deletion intent in the same commit as primary cleanup and clears a copy lease, so a delayed exporter cannot resurrect the archive row. The AES-GCM archive keeps paths/manifests encrypted under opaque HMAC locators, and its authenticated tombstone ledger survives after ciphertext deletion; restore treats that ledger as a deny list and fails closed on corruption, missing bytes, wrong environment/scope/key, or point mismatch. Ordinary and evidence exports have different secrets, neither external caller holds a service key, and production RPO/RTO is never claimed from a local drill.
