...
- Confirm whether the active checkpoint is approved. Phase 1 and Phase 2 are implemented on `codex/friend-first-rebaseline`. Checkpoints 1A, 1B, 2A, and 2B are promoted to the reused hosted-development project with green hosted gates; the 1B Auth-email gate is closed. Checkpoint 2C — avatars, the shared reserved-object uploader, the media-cleanup outbox, and the `reconcile-operations` worker — is implemented and green **locally only**, under an approval that explicitly stopped before creating hosted buckets, Vault secrets, or Cron schedules and before deploying Edge Functions. Do not create or deploy any of those without the separate promotion approval quoted in Section 31. Physical-iPhone acceptance for both 1B and 2C remains open.
