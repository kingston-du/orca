...
Implementation is underway. **Phase 0 — Expo engineering baseline** is complete: the app runs in the iOS Simulator and the Home / Camera / Memories / Settings shell exists. The private GitHub repository is connected as `origin`, and `.env` is not tracked. The toolchain is pinned to Node `24.14.1` and npm `11.11.0`; Supabase CLI `2.109.1` and Expo Doctor `1.20.1` remain exactly pinned. The local Supabase stack runs Postgres `17.6` and cleanly replays all nine current migrations. The first eight are applied to hosted development with matching history, all 247 remote assertions passing, and no hosted Security/Performance Advisor warnings or errors. The local-only ninth migration implements invitation-gated signup with transient Auth metadata, hash-backed capacity reservations, verification-time membership, fresh-code recovery, reservation-aware ordinary redemption, and a database-enforced onboarding boundary. The local seed alone enables `development_open`; hosted/production defaults to `invitation_required`. A real local Auth/Mailpit flow and clean reset pass, along with all 285 local assertions, database lint/advisors, exact generated types, all 56 app tests, an iOS export, and Expo Doctor 20/20. The app has a Circle hub and create/join/detail flows with TanStack Query ownership, full member/invite administration, explicit destructive confirmations, exact exited/deleted cache removal, and ephemeral raw invite tokens. The exposed onboarding/Circle/signup RPCs are security invoker and delegate only narrow atomic writes to independently validating helpers in the unexposed `private` schema. The temporary legal text remains founder-only. Numbered tutorials in `docs/course/` explain completed checkpoints in depth. The Supabase client retains encrypted session storage and one AppState refresh owner; Auth state remains outside TanStack Query and identity transitions clear all current user-scoped data. EAS project `@kingstondu/orca` (`dc295559-844a-48f4-924a-e653c31602cd`) and Android build `38ec9b82-8472-4e99-9b04-fa8bbdb1b013` remain configured; physical acceptance is deferred. Apple Developer enrollment/physical iPhone work remains deferred but required before external iOS testing. GitHub's current high/moderate alerts remain build/test-tooling-only with no Expo-compatible nonbreaking fix; do not force-fix, downgrade Expo, or install `uuid` directly. There is no production Supabase project.

The active implementation milestone is **Phase 3 — Circles, membership, and invitations**. Its Circle database/app lifecycle is hosted and verified, and invitation-gated signup is fully implemented/verified locally. The linked CLI, ignored app URL, project metadata, health, and dry run all identify `orca-dev` (`evuqnmvcnqhkzkitszqp`); the dry run names only `20260729072231_add_invitation_gated_signup.sql` and applies nothing. Explicit approval is the next promotion gate. Hosted deployment must then configure the Before User Created Auth hook, rerun remote assertions/advisors, and complete the remaining simulator visual passes. Phase 2's hosted SMTP/bot protection, final legal copy, and physical-device acceptance remain pre-external-tester gates. Do not amend applied migrations or make ad-hoc dashboard changes.
