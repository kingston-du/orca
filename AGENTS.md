...
- V1 release gates still include public privacy/legal review, Sentry source maps plus a symbolicated release crash, universal links/AASA, App Store metadata/privacy answers, and separately approved build/upload/distribution. No production Supabase project exists.
- Section 27 V1.1A's live 24-hour Recent correction is green locally and promoted to hosted development. The founder explicitly waived its pre-promotion physical clock-boundary/background-resume pass on 2026-08-09; that device evidence remains deferred rather than passed.
- The current TestFlight review build receives the hosted 24-hour filter on its next Home fetch without a binary update. It does not contain V1.1A's exact in-session boundary timer, which remains part of the next distributed V1.1 binary. V1.1B is the next local checkpoint. No plan or local completion authorizes later hosted/provider/function/native/release mutation or distribution.
