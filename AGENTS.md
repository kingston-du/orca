...
Build Splotty into a production-quality private, friend-first iOS photo app while keeping the architecture simple enough for an initial ~100-user beta.

Splotty's tagline is:
...
- Confirm whether the active checkpoint is approved. Hosted and local development share all eighteen migrations through `20260810120000_splotty_development_legal_documents.sql`; the Splotty Auth templates are promoted and a second config comparison reports no drift. The approved 2026-08-03 dependency promotion applied Phase 8, 9A, and 9B, deployed the notification/deletion-aware `reconcile-operations` worker plus fail-closed `backup-media`, and left both Cron jobs healthy. The founder declined a separate archive provider, scheduler, credentials, monitoring, remote drill, and RPO/RTO program for V1; hosted sets `ORCA_BACKUP_OPERATIONS_ENABLED=false`, has no backup secrets/snapshots, and makes no Storage-recovery or RPO/RTO claim. The founder approved Checkpoint 9D and chose the existing hosted Free development project for the first internal TestFlight build; a separate production project and plan are deferred until the app is proven. Phase 8 still needs the Expo push access token, a signed physical-device build/install, and two-device acceptance. Phase 7 and the SQLSTATE correction remain promoted; TOTP enrol/verify and the real hosted AAL2 moderation drill remain proven. Exact disposable cases, evidence objects, operators, and Auth users were removed; the append-only redacted audit actions remain by design. Phase 6 is complete and promoted, and Phase 5 is complete, promoted, and physically accepted. Two founder-directed product changes are settled and both say the same thing: an author sees their own posted Recent Moments on Home (Checkpoint 5B), and an author sees their own Moments in Highlights alongside their friends' (Phase 6). **Treat "a person's own content appears wherever everyone else's is shown" as the standing rule, not as two exceptions.** Phase 5 owes no device gate. Phase 6's physical-feel item is outstanding. `expo-haptics` is installed and called; the regenerated Splotty development target contains it, but physical feel is still unaccepted. Physical-iPhone acceptance for 1B's remaining items, 2C, Phase 3's capture-metadata fixtures, and Phase 4's interruption/network publish cases remains open, and production resources still do not exist. Phase 7's four safety decisions are **approved (2026-08-03)** and implemented as written in [the safety policy decisions](docs/operations/2026-08-02-safety-policy-decisions.md). `kingstonduprojects@gmail.com`, the supplied Sentry DSN, hosted URL/publishable key, and correct release labels are configured in ignored local state and EAS development/preview. Sentry organization/project slugs plus auth token, a symbolicated native crash, and the permanent founder-held moderator/TOTP identity remain release operations. Checkpoint 9D's external-beta legal text is **written (2026-08-04)**: the four development documents are retired in favour of one agreement, `legal/beta-2026-08-04/terms.md`, accepted through a single onboarding control. It names Kingston Du, an individual, California, with `kingstonduprojects@gmail.com` as the published contact and California governing law — all founder decisions. Sign in with Apple was declined and is compliant to omit, because Splotty offers no other third-party login. The twenty-first migration carrying this is green locally and **not yet promoted**; promoting it deliberately makes every existing hosted user stale-legal until they re-accept. What remains is not engineering: a lawyer's review and a public privacy-policy URL for App Store Connect. The App Store listing metadata, App Privacy label answers, age-rating answers, and review notes are **written and gated (2026-08-18)** in [the listing document](docs/operations/2026-08-18-app-store-listing-and-privacy-answers.md); `npm run store:check` binds them to the agreement's version and digest and enforces every field's character limit, so replacing the legal text fails until the answers are re-derived. Two things there are the founder's: the honest questionnaire answers compute to 13+ while the agreement says 18+, and the privacy-policy/support URLs, screenshots, and demo account are left empty on purpose — an invented value is worse than an empty one. Do not invent missing identities or credentials, and never edit an accepted legal document in place — an acceptance is bound to its content hash, so a replacement is a new version, a new hash, and a new acceptance.

- Checkpoint 9C added one rule worth carrying forward: **an approved design is an input, not an authority.** Three neutral roles in the pre-rename "Orca Screens" design composite to roughly 3.6:1, 2.8:1, and 2.4:1 on the canvas, and the photo scrim was drawn at the exact alpha `design.ts` already documented as breaking its muted pairing; each was darkened before adoption and `npm run contrast:check` now recomputes all twelve pairings from the token file so none can regress. Two structural rules came out of the same checkpoint. A presentational component must not reach for navigation — `ScreenHeader` takes an `onBack` callback from its route, because `useRouter` in the shared header put `expo-router` into every screen's import graph and corrupted React Native's native component registry under Jest, surfacing as an unrelated `Switch` rendering `undefined`. And an alert role belongs on the text, never on a container that also holds buttons, which a screen reader would otherwise swallow. Every colour now lives in `src/constants/design.ts`; no other source file may contain a hex literal. The tab bar is the single surface permitted to cap Dynamic Type. `expo-haptics` is installed and present in the regenerated target; a signed physical install remains.

- The 2026-08-03 founder review of 9C added a geometry rule: **move the smallest visual owner that can solve the problem.** The scrim is one short native continuous gradient: its eased alpha stays subtle at the top and keeps changing through the bottom, because stacked translucent views create stripes and a repeated-alpha tail creates a visible bar. Both overlay text roles are white so the softened curve remains legible. Home and the bounded restored-draft preview may use reversible `cover` presentation crops, while scrollable composer/detail frames use verified natural media geometry with `contain`, preserving the complete file without rails. Elapsed capture time owns recent phrasing; the stored capture offset owns an older photo's calendar date. A field's scroll view owns keyboard insets; a translated sheet does not lift its whole panel in response to the keyboard. Preserve those boundaries when refining the same surfaces.

- The 2026-08-03 founder device-feedback pass added three rules worth carrying forward. **A default that only fires on a browser event does not fire on a phone.** TanStack's `refetchOnWindowFocus` is on by default and had never once run, because its focus signal is a DOM `visibilitychange`; every "this screen doesn't update until I leave and come back" report traced to that single missing subscription rather than to any screen. When a library's default behaviour is load-bearing, prove the signal it depends on exists in this runtime. **The surface that takes delivery of a thing owns retiring it.** Publishing now leaves the composer at the first byte, so progress, cancel, needs-review, and unknown-outcome travel to Home with the author rather than staying on a screen they have left; Home draws the Moment from the local file and dismisses the attempt when the server's own row arrives. Moving a first-class state to another surface is fine; dropping it because the author walked away is not. **Grouping is a rule about strangers.** Hearts were coalesced on a twelve-minute delay, which in a private feed of a few friends read as "likes do not notify" and then as a summary of something already forgotten. Before batching anything, ask whether the volume that justified batching exists at this product's scale.

- Checkpoint 9B added one rule worth carrying forward if the optional archive is ever enabled: **a deletion tombstone outranks both an in-flight backup copy and an older database recovery point.** The cleanup-row trigger records backup deletion intent in the same commit as primary cleanup and clears a copy lease, so a delayed exporter cannot resurrect the archive row. The AES-GCM archive keeps paths/manifests encrypted under opaque HMAC locators, and its authenticated tombstone ledger survives after ciphertext deletion; restore treats that ledger as a deny list and fails closed on corruption, missing bytes, wrong environment/scope/key, or point mismatch. V1 has no archive caller or production recovery claim; do not re-enable `ORCA_BACKUP_OPERATIONS_ENABLED` without the complete credential/provider/drill scope.

- Checkpoint 9A added one rule worth carrying forward: **Auth is the last irreversible deletion step, and the database proves it.** The request changes `account_states` to `deleting` in the same transaction that records a noncascading job/receipt, so stale JWTs lose ordinary access immediately. Bounded graph/media passes release their leases as useful work; every known and prefix-orphaned byte must reach the Storage-proof outbox; and the relational child barrier clears before `auth.admin.deleteUser` runs. The client encrypts a 32-byte capability before requesting, sends only its SHA-256, and the signed-out status route never puts the bearer value in a route or cache key. A receipt says `complete` only after a database check finds no Auth row.

- Phase 8 added one rule worth carrying forward: **a notification is authorized twice, and the second time is the one that counts.** A job is a claim that something happened, not permission to tell anyone. Producing and suppressing are row triggers on the tables the domain transactions already write — never edits to `apply_friend_command`, `finalize_moment_upload`, or `apply_moderation_action`, whose promoted bodies must stay byte for byte — so the fact and its job are one commit and no future writer can forget the suppression. `private.notification_block_reason` re-derives eligibility, blocks, generation, visibility, and preference immediately before sending, reusing the existing predicates rather than restating them. Nothing user-facing is stored on a job: the six generic strings live in `supabase/functions/_shared/push-delivery.ts`, and the payload is an event UUID plus at most one opaque route UUID.

- Phase 7 added one rule worth carrying forward: **never raise a class-40 SQLSTATE from an RPC a client reaches through PostgREST.** PostgREST retries `40001` rather than returning it, so the caller sees a gateway timeout instead of a conflict. Use `55000` for "this changed under you". Corrective migration `20260805180000_postgrest_conflict_sqlstates.sql` moves every older friend, avatar, caption, and Moment-finalization optimistic conflict to `55000`; pgTAP asserts that no stored `public` or `private` function retains `40001`.
- The repository-wide quality gates exposed during the SQLSTATE correction are closed in the Phase 7 completion checkpoint. `expo-font` is now a direct SDK-compatible dependency, so Expo Doctor is 20/20 without adding an unused font config plugin. The retained Jest handles came from test QueryClient garbage-collection timers and eager Sentry runtime initialization; deterministic infinite test `gcTime` plus lazy Sentry loading when no DSN is configured make the whole suite — 445 assertions across 58 suites through 9B — exit cleanly under `--detectOpenHandles`.
...
- Foreground identity/access revalidation remains mandatory. By explicit founder decision after 9C review, do not draw the former opaque cover on every inactive/background transition; take the screen only when foreground revalidation fails. Keep this distinct from the cross-account cache rule, which must still prevent an old user's bytes or rows from flashing after identity changes. This does not prevent user screenshots.
...
3. distinguish platform limit, current policy, measured Splotty evidence, and planning assumption;
4. record important links/decision in `PROJECT.md`.

Official quickstarts are setup references, not automatically Splotty's production target. Verify SQL grants/Data API exposure separately from RLS. Recheck costs before purchase and App Review requirements before beta/submission.
...
- focused excerpts/links to real Splotty code, not entire files;
