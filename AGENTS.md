...
- Confirm whether the active checkpoint is approved. Phases 1 and 2 are complete and promoted on `codex/friend-first-rebaseline`: local and hosted development run the same six-migration history, and hosted carries the private `avatars` bucket, the `finalize-avatar` and `reconcile-operations` Edge Functions, both Vault secrets, `pg_cron`/`pg_net`, and both Cron schedules, all verified. Phase 3 is implemented locally and added no backend surface — capture evidence, the composer reducer, and one scoped recoverable draft. Phase 4 is implemented locally and does add backend surface: local now runs a **seventh** migration with the `moment-media` bucket and the `finalize-moment` function, so local and hosted have diverged. Promoting Phase 4 to hosted is a separate approval it has not received, and Checkpoint 5A is not authorized either; both precise approval sentences are in Section 31. Physical-iPhone acceptance for 1B, 2C, Phase 3's capture-metadata fixtures, and Phase 4's publish pass remains open, and production resources still do not exist.
