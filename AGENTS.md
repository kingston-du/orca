...
Implementation is underway. **Phase 0 — Expo engineering baseline** is complete: the app runs in the iOS Simulator and the Home / Camera / Memories / Settings shell exists. The private GitHub repository is connected as `origin`, and `.env` is not tracked. The toolchain is pinned to Node `24.14.1` and npm `11.11.0`; Supabase CLI `2.109.1` and Expo Doctor `1.20.1` remain exactly pinned. The local Supabase stack runs Postgres `17.6`; the first nine migrations are applied to hosted development with matching history, 285/285 remote assertions, and clean advisors. The July 29 product decision supersedes the ninth migration's signup gate: Auth signup is permanently open so any organizer can create an account and start a separate Circle, while Circle membership remains invite-only. Corrective migration `20260730044326` and the app cleanup are locally complete without amending applied history; all ten migrations replay, all 259 local database assertions pass, database lint/types are exact, all 50 app tests and an iOS export pass, and Expo Doctor is 20/20. The exact-target dry run identifies only that corrective migration for healthy linked `orca-dev`. Default Circle invitations last seven days and allow up to 10 joins for group-chat sharing. Expo SDK 57 packages are on the current compatible patch set. The temporary legal text remains founder-only. The encrypted session/Auth boundaries, EAS setup, and deferred Apple/physical acceptance remain unchanged. There is no production Supabase project.

The active implementation milestone is **Phase 3 — Circles, membership, and invitations**. The open-signup correction is locally verified and awaiting its explicit hosted promotion gate. For the hosted change, first disable the obsolete Before User Created hook while the old database trigger still denies code-free signup, then apply only `20260730044326`, verify open signup plus invite-only Circle joining, rerun hosted tests/advisors, complete the short simulator acceptance pass, and begin Phase 4. Do not amend applied migrations or make unrelated dashboard changes.
