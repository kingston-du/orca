...
Implementation is underway. **Phase 0 — Expo engineering baseline** is complete: the app runs in the iOS Simulator and the Home / Camera / Memories / Settings shell exists. The private GitHub repository is connected as `origin`, and `.env` is not tracked. The toolchain is pinned to Node `24.14.1` and npm `11.11.0`. Expo is explicitly iOS/Android-only, iPad support is disabled, direct web configuration/script/dependencies are removed, and verified production/development identities resolve separately through `app.config.js`. The direct dependency audit is complete: unused template packages are removed and current `@supabase/supabase-js` `2.110.8` plus `react-native-url-polyfill` `4.0.0` are exactly pinned. Supabase CLI `2.109.1` and Expo Doctor `1.20.1` are exactly pinned. The Docker-backed local stack is running, resets from version-controlled inputs, and answers direct queries on Postgres `17.6`. The security, account/profile, and legal/18+ onboarding foundations are applied to hosted development; all five migration histories match, all 95 remote assertions pass, and hosted Security and Performance Advisors report no issues. The exposed onboarding RPC is security invoker and delegates only its privileged atomic write to a narrowly granted helper in the unexposed `private` schema. The temporary legal text is explicitly founder-only and must be replaced before external testing. GitHub CI runs app quality, component tests, and clean local-database jobs. Expo-compatible Jest/RNTL and one app-level TanStack Query provider are installed; Auth remains outside the query cache, native foreground focus is connected, and every real identity transition clears all current user-scoped client data through one centralized boundary. Separate sign-in/sign-up routes use a tested shared form with validation, loading/double-submit protection, safe error mapping, accessibility, and native credential semantics; their Expo Go visual acceptance is complete. Unconfirmed sign-up routes to a tested six-digit verification screen with input filtering, duplicate-request protection, safe missing-state recovery, and a 60-second resend cooldown. Password recovery uses a generic email response, in-app recovery code, and confirmed replacement password; `PASSWORD_RECOVERY` keeps the temporary session inside Auth routes until `USER_UPDATED`, and failure removes it locally. Authenticated users load their self-only profile and current acceptance references through TanStack Query; incomplete or stale accounts stay in onboarding until the atomic RPC records a display name and all four exact document fingerprints. Settings shows the profile/Auth identity and performs explicit local-device sign-out; `SIGNED_OUT` drives protected routing and user-scoped query cleanup. Real local Auth/Mailpit/Data API flows prove confirmation, recovery, and onboarding behavior. All 44 app tests, all 95 local pgTAP assertions, and an iOS export bundle pass. Numbered tutorial lessons in `docs/course/` explain each completed checkpoint’s mental model, runtime flow, important syntax, security boundary, tests, and evidence. The README documents explicit local versus linked workflows, the dependency graph and Dependabot alerts are enabled, and green CI is the documented `main` safeguard because this private personal-repository plan cannot enforce branch rules. The Supabase client uses an isolated Expo Crypto AES-GCM adapter that stores only ciphertext in AsyncStorage and its random key in SecureStore, fails closed on missing/corrupt data, serializes same-key operations, and preserves the existing single AppState refresh owner. EAS project `@kingstondu/orca` (`dc295559-844a-48f4-924a-e653c31602cd`) has a committed development profile and public development environment. Android build `38ec9b82-8472-4e99-9b04-fa8bbdb1b013` finished successfully from commit `18765b7` with an Expo-managed keystore; physical Android acceptance is deferred. Apple Developer enrollment and the physical-iPhone EAS build are intentionally deferred while Apple's account process is delayed; this must close before any external iOS tester or TestFlight build. Orca is iOS-first during active development. Protected routes live under `(app)` and signed-out routes under `(auth)`; one Auth provider restores encrypted sessions via the synchronous `INITIAL_SESSION` event, exposes local sign-out, and drives stable `Stack.Protected` guards. The runtime audit baseline remains 11 moderate and zero high/critical findings; Jest/RNTL add development-only advisories that npm 11 also lists through optional peer edges under `--omit=dev`, although the production install graph excludes the test packages. Do not force-fix or install `uuid` directly. A hosted Supabase development project exists, but there is no production project.

The active implementation milestone is **Phase 3 — Circles, membership, and invitations**. Phase 2's simulator-based Auth, recovery, onboarding, Settings, local sign-out, and complete current user-state cleanup implementation is verified. Its hosted Auth settings, custom SMTP/template promotion, bot protection, final legal copy, quick remaining Expo Go visual passes, and physical-device acceptance remain explicit pre-external-tester gates. The next checkpoint builds the Circle, membership, and invitation database foundation locally with its grants, RLS, invariants, and negative authorization tests. Do not amend any applied migration, make ad-hoc dashboard changes, or promote the new migration before its local gate and exact-target review.
