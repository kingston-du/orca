...
- Confirm whether the active checkpoint is approved. Phases 1 through 4 and Checkpoint 5A are complete and promoted on `codex/friend-first-rebaseline`: local and hosted development run the same **nine**-migration history, and hosted carries the private `avatars` and `moment-media` buckets, the `finalize-avatar`, `finalize-moment`, and `reconcile-operations` Edge Functions, both Vault secrets, `pg_cron`/`pg_net`, and both Cron schedules, all verified against real HTTP. Checkpoint 5B is not authorized until the founder gives the precise approval in Section 31. Physical-iPhone acceptance for 1B, 2C, Phase 3's capture-metadata fixtures, Phase 4's publish pass, and 5A's left=older/VoiceOver/Dynamic Type pass remains open, and production resources still do not exist.
