...
- Confirm whether the active checkpoint is approved. Phases 1 through 4 and Checkpoint 5A are complete and promoted on `codex/friend-first-rebaseline`: local and hosted development run the same **nine**-migration history, and hosted carries the private `avatars` and `moment-media` buckets, the `finalize-avatar`, `finalize-moment`, and `reconcile-operations` Edge Functions, both Vault secrets, `pg_cron`/`pg_net`, and both Cron schedules, all verified against real HTTP. Checkpoint 5B is not authorized until the founder gives the precise approval in Section 31. 5A's own physical-device pass is complete (left=older accepted, VoiceOver/Dynamic Type/reduced-motion/privacy-shield/airplane-retry all verified on the founder's iPhone) and found two device-only defects, both fixed and pushed at `51ae53a` — see Section 27. Physical-iPhone acceptance for 1B's remaining items, 2C, Phase 3's capture-metadata fixtures, and Phase 4's interruption/network publish cases remains open, and production resources still do not exist.
