...
Implementation is underway. **Phase 0 — Expo engineering baseline** is complete: the app runs in the iOS Simulator and the Home / Camera / Memories / Settings shell exists. The private GitHub repository is connected as `origin`, and `.env` is not tracked. The toolchain is pinned to Node `24.14.1` and npm `11.11.0`; Supabase CLI `2.109.1` and Expo Doctor `1.20.1` remain exactly pinned. The Docker-backed local stack runs Postgres `17.6` and cleanly replays all six local migrations. The first five migrations through legal/onboarding are applied to hosted development; all 95 remote assertions pass and hosted Security/Performance Advisors are clean. Local-only migration `20260728233731` adds the Circle/membership/invite foundation, atomic creator-as-admin RPC, nonrecursive membership helpers, shared-Circle profile reads, least-privilege grants, and generated types. All 157 local pgTAP assertions, local lint/advisors, a real local Auth/Data API creator-versus-outsider flow, all 44 app tests, and Expo Doctor 20/20 pass. The migration is not yet promoted. The exposed onboarding and Circle RPCs are security invoker and delegate only narrow atomic writes to independently validating helpers in the unexposed `private` schema. Invite hashes are stored but not client-readable; invite creation/redemption and membership mutations are not implemented yet. The temporary legal text remains founder-only. Numbered tutorials in `docs/course/` explain completed checkpoints in depth. The Supabase client retains encrypted session storage and one AppState refresh owner; Auth state remains outside TanStack Query and identity transitions clear all current user-scoped data. EAS project `@kingstondu/orca` (`dc295559-844a-48f4-924a-e653c31602cd`) and Android build `38ec9b82-8472-4e99-9b04-fa8bbdb1b013` remain configured; physical acceptance is deferred. Apple Developer enrollment/physical iPhone work remains deferred but required before external iOS testing. The runtime audit baseline remains 11 moderate and zero high/critical findings; do not force-fix or install `uuid` directly. There is no production Supabase project.

The active implementation milestone is **Phase 3 — Circles, membership, and invitations**. Its first database foundation is locally verified. The next checkpoint reverifies linked project `orca-dev` (`evuqnmvcnqhkzkitszqp`), dry-runs only migration `20260728233731`, and requires an explicit gate before hosted promotion; afterward, rerun all 157 remote assertions and both hosted advisors. Invite creation/preview/redemption and membership/admin lifecycle follow as a separate concurrency-focused checkpoint. Phase 2's hosted Auth/SMTP/bot protection, final legal copy, remaining visual passes, and physical-device acceptance remain pre-external-tester gates. Do not amend applied migrations or make ad-hoc dashboard changes.
