...
- Confirm whether the active checkpoint is approved. Phase 7 and the post-Phase-7 SQLSTATE correction are **implemented locally and not promoted**: local runs thirteen migrations, hosted still runs eleven, and `moderate-report` is not deployed. Phase 6 is complete and promoted on `codex/friend-first-rebaseline`, and Phase 5 is complete, promoted, and physically accepted. Hosted carries the private `avatars` and `moment-media` buckets, the `finalize-avatar`, `finalize-moment`, and `reconcile-operations` Edge Functions, both Vault secrets, `pg_cron`/`pg_net`, and both Cron schedules, all verified against real HTTP. Two founder-directed product changes are settled and both say the same thing: an author sees their own posted Recent Moments on Home (Checkpoint 5B), and an author sees their own Moments in Highlights alongside their friends' (Phase 6). **Treat "a person's own content appears wherever everyone else's is shown" as the standing rule, not as two exceptions.** Phase 5 owes no device gate — the founder ran and accepted 5B's pass along with the items the post-5A restyle had reopened. Phase 6's own physical-feel item is outstanding, and haptics are deferred by founder direction so a later native rebuild can carry several changes at once. Physical-iPhone acceptance for 1B's remaining items, 2C, Phase 3's capture-metadata fixtures, and Phase 4's interruption/network publish cases remains open, and production resources still do not exist. Phase 7's four safety decisions (operator identity, retention, appeal route, support contact) are **approved (2026-08-03)**; the code already behaves as written in [the safety policy decisions](docs/operations/2026-08-02-safety-policy-decisions.md). Remaining founder gates: two hosted Auth settings (TOTP enrol/verify — leaked-password protection stays disabled, Pro-tier only), the support mailbox, hosted promotion plus operator provisioning, and the Sentry project. Do not moderate, provision an operator, or promote without those approvals.
...
- The repository-wide rerun for that correction exposed one unrelated pre-existing gate: Expo Doctor is 19/20 because direct dependency `expo-symbols` declares `expo-font` as a peer while Orca has `expo-font` only transitively. The installed version is already `57.0.1` and the native pod is already present; declaring the peer directly is a separate dependency-hygiene change, not part of the SQLSTATE checkpoint. The 393 Jest assertions pass, but that existing runner also retains an asynchronous handle after completion and must be interrupted. Do not report either check as green until its owning fix lands.
