...
- Confirm whether the active checkpoint is approved. Phase 6 is implemented locally on `codex/friend-first-rebaseline` and Phase 5 is complete, promoted, and physically accepted. **Local development runs an eleven-migration history; hosted development runs ten.** The one unpromoted migration is `20260804120000_reactions_and_highlights.sql`, and its hosted promotion is a separate approval that has not been given — do not promote it without one. Hosted carries the private `avatars` and `moment-media` buckets, the `finalize-avatar`, `finalize-moment`, and `reconcile-operations` Edge Functions, both Vault secrets, `pg_cron`/`pg_net`, and both Cron schedules, all verified against real HTTP. Checkpoint 5B was approved together with one founder-directed product change — an author sees their own posted Recent Moments on Home — and Phase 5 now owes no device gate: the founder ran and accepted 5B's pass along with the items the post-5A restyle had reopened. Phase 6's own physical-feel item is outstanding. Physical-iPhone acceptance for 1B's remaining items, 2C, Phase 3's capture-metadata fixtures, and Phase 4's interruption/network publish cases remains open, and production resources still do not exist.
