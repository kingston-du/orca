...
- Confirm whether the active checkpoint is approved. Phase 5 is complete on `codex/friend-first-rebaseline`. Phases 1 through 4 and Checkpoint 5A are promoted; local development runs a **ten**-migration history and hosted still runs the nine-migration one, so `20260803120000_home_sessions_and_history.sql` is the single unpromoted migration. Hosted carries the private `avatars` and `moment-media` buckets, the `finalize-avatar`, `finalize-moment`, and `reconcile-operations` Edge Functions, both Vault secrets, `pg_cron`/`pg_net`, and both Cron schedules, all verified against real HTTP. Checkpoint 5B was approved together with one founder-directed product change — an author now sees their own posted Recent Moments on Home — and its hosted promotion and physical-device pass are approved and outstanding. 5A's own physical-device pass is complete (left=older accepted) and found two device-only defects, both fixed at `51ae53a`; the post-acceptance restyle means most of that pass must be re-run against the current geometry — see Section 27. Physical-iPhone acceptance for 1B's remaining items, 2C, Phase 3's capture-metadata fixtures, Phase 4's interruption/network publish cases, and 5B's 50-card runs remains open, and production resources still do not exist.
