...
Implementation is underway. **Phase 0 — Expo engineering baseline** is complete: the app runs in the iOS Simulator and the Home / Camera / Memories shell exists. The private GitHub repository is connected as `origin`; local `main` tracks `origin/main` through encrypted-session commit `9405831`, and `.env` is not tracked. The toolchain is pinned to Node `24.14.1` and npm `11.11.0`. Expo is explicitly iOS/Android-only, iPad support is disabled, direct web configuration/script/dependencies are removed, and verified production/development identities resolve separately through `app.config.js`. The direct dependency audit is complete: unused template packages are removed and current `@supabase/supabase-js` `2.110.8` plus `react-native-url-polyfill` `4.0.0` are exactly pinned. Supabase CLI `2.109.1` and Expo Doctor `1.20.1` are exactly pinned. The Docker-backed local stack is running, resets from version-controlled inputs, and answers direct queries on Postgres `17.6`. The committed security-foundation migration establishes the `private` schema and least-privilege defaults; a clean reset applies it, all 16 pgTAP assertions pass locally and remotely, hosted Security and Performance Advisors report no warnings, and the probes leave no residue. Generated public-schema database types are committed and the Supabase client uses `createClient<Database>()`. GitHub CI runs app quality and clean local-database jobs; its first push run passes both jobs. The README documents explicit local versus linked workflows, the dependency graph and Dependabot alerts are enabled, and green CI is the documented `main` safeguard because this private personal-repository plan cannot enforce branch rules. Expo compatibility, TypeScript, zero-warning lint, formatting, and all 20 `expo-doctor` checks pass. The Supabase client now uses an isolated Expo Crypto AES-GCM adapter that stores only ciphertext in AsyncStorage and its random key in SecureStore, fails closed on missing/corrupt data, serializes same-key operations, and preserves the existing single AppState refresh owner. It compiles and bundles in the iOS Simulator but still needs physical-device development-build acceptance. EAS project `@kingstondu/orca` (`dc295559-844a-48f4-924a-e653c31602cd`) is created under the verified Expo account and linked; its development profile/environment and first builds remain in progress. The Auth shell lacks real sign-in behavior and a protected session gate. The production-oriented audit remains 11 moderate and zero high/critical findings when dev dependencies are omitted; do not force-fix or install `uuid` directly. A hosted Supabase development project exists, but there is no production project.

The active milestone is **Phase 1 — Recoverable engineering and backend foundation**. Phase 1B and the encrypted-session implementation are complete through pushed commit `9405831`. GitHub CI run `30227256433` passes both App quality and Database quality after a transient public-ECR rate limit cleared on rerun. The pinned Supabase CLI remains linked only to healthy hosted development project `orca-dev` (`evuqnmvcnqhkzkitszqp`, `ca-central-1`), and hosted Security and Performance Advisors are warning-free. Phase 1C now has exact linked EAS target `@kingstondu/orca`. Correct `eas.json` by placing the SDK image in each platform block, upload only the public development Supabase values, validate and commit the profile, then verify restore, refresh, sign-out, reinstall, kill/relaunch, and foreground/background behavior on physical iOS and Android. Do not amend either applied migration, make ad-hoc dashboard changes, or resume Auth UI before physical session acceptance passes.
