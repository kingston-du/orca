...
Implementation state: Phase 5 is complete and promoted. Local and hosted development run the same ten-migration history, and hosted carries both media buckets, all three Edge Functions, both Vault secrets, and both Cron schedules, all verified against real HTTP. Checkpoint 5B's physical-device pass is the only gate it still owes.
...
These checks validate Checkpoints 1A, 1B, 2A, 2B, 2C, and Phases 3, 4, and 5 on both local and hosted development. Reactions, safety surfaces, notifications, and release infrastructure remain planned.
...
Reactions; Highlights; notification infrastructure; reports and moderation evidence; friend-first deletion; production media backup; universal links; monitoring; Maestro flows; and the beta release process are all planned only.
...
Checkpoint 5B was explicitly approved together with one founder-directed product change — that an author sees their own posted Moments in Recent on Home — and is implemented locally; its hosted promotion and physical-device pass are separately approved and outstanding. Checkpoint 1A was explicitly approved and implemented locally. Checkpoint 1B was explicitly approved with a founder-directed change of method — reuse the existing hosted-development project and waive rollback — and is implemented apart from the Auth-email gate recorded in Section 1. Checkpoints 2A and 2B are implemented and promoted. Checkpoint 2C was implemented under an approval that stopped before every hosted resource, then promoted under a separate explicit approval; promotion exposed and fixed a real dispatcher defect recorded below. Phases 3 and 4 were each explicitly approved. Phase 3 added no backend surface; Phase 4 was implemented locally and then promoted to hosted development under a separate explicit approval, and promotion surfaced one real advisory defect — duplicate permissive read policies — fixed by a corrective migration rather than by amending applied history. Later checkpoints remain planned and require the approvals stated in their phase and in Section 31.
...
- Author avatar/name and the friendly capture time are drawn **on** the photo, over an opaque scrim band at its foot, so a card reads as one photographic object. The caption stays outside the image: it is the only element with no length bound, and arbitrary-length text over a face is how an overlay design fails. Above a large-text threshold the overlay is abandoned entirely and identity returns above the photo, where it has the full width to wrap. The scrim is a solid band, never a gradient that assumes the photo beneath it is dark, and its alpha is load-bearing — both text roles are specified against the scrim composited over pure white, the worst case a photo can present. None of this changes VoiceOver order, which remains author → exact capture date/time → photo → caption.
...
- Seen is marked after at least 85% visibility for one second while Home is focused and the app active; detail/reaction also marks it. On the deck the settled current card _is_ that condition — it is the only card at full size and opacity, and a card still moving has not settled — so the dwell timer starts on settle and is cancelled by any change of card, focus, or foreground. Batch idempotent writes within roughly two seconds/on leave/background. Do not persist an offline seen queue: a record the server never received belongs to a session the viewer has already left, and replaying it later would move the unseen partition of a session that no longer exists.
...
- Large fixed photo containers, approximately 4:5 where the screen permits, with `contain` on a warm neutral backing so arbitrary ratios remain legible. Caption and actions stay outside the image; bounded identity metadata may sit on it over a scrim, and returns outside it at large text sizes.
...
### Phase 4 — One-Moment trusted publication vertical (**complete and promoted; physical-device publish deferred**)
...
Phase 5 is complete and promoted. Local and hosted development run the same ten-migration history, and hosted carries both media buckets, all three Edge Functions, both Vault secrets, `pg_cron`/`pg_net`, and both Cron schedules, all verified against real HTTP. There is no divergence between the two environments and no unpromoted migration.

The next action is **Phase 6 — Heart, Superheart, and Highlights**, exactly as scoped in Section 27. Reactions are the first surface where two people write to the same row at the same time, so its risk is concurrency rather than authorization: the ordered account locks, the serialized three-per-rolling-day Superheart quota, the no-refund/no-double-consume rule, and the viewer-filtered counts that must not leak a hidden identity through a number.

Checkpoint 5B's physical-device pass is authorized and outstanding, and it is the honest prerequisite for trusting Phase 6's "physical feel" gate: reaction controls are added to a deck whose current geometry has not yet been measured on hardware.

Checkpoint 5A's own device-acceptance item is closed, and 5B's is open — see Section 27. The 5A physical-device session also exercised, but did not fully close, two of the four device gates below: a real signup/onboarding regression and the app-switcher privacy shield (part of 1B), and a real device successfully reserving, uploading, and finalizing a Moment (part of Phase 4's publish acceptance). Their remaining sub-items — background/process-death/named-network transfer, avatar upload acceptance, and the HEIC/iCloud capture-metadata fixtures — stay open. The 5A restyle also left VoiceOver, Dynamic Type, scrim contrast, reduced motion, and the release-build timing budgets to be re-run against the current deck geometry, and 5B's own three 50-card memory/performance runs and signed-URL residual checks join them.

Four device gates remain open and are independent of that approval:

- **Physical-iPhone smoke (from 1B).** A rebuilt development client must exercise absence of an old-user frame on foreground and friend-first camera permission/readiness copy. Hosted email now works, so nothing blocks this but device time.
...
- **Physical-iPhone publish acceptance (from Phase 4).** Cancel mid-transfer, a backgrounded transfer that outlives the foreground, process death between upload and finalize, publishing on named Wi-Fi and LTE profiles, and the media memory ceiling from Section 17. A basic reserve → upload → finalize round trip is now proven on device; these interruption/network cases are not.
