...
- Hosted migration `20260729072231` implements invitation-gated signup. Auth receives a validated raw invite only as transient metadata; a `BEFORE` trigger strips it before storage and leaves a server-derived marker long enough for the authoritative Auth insert trigger to reserve hash-backed capacity. Email confirmation claims membership, invalidated reservations route to an authenticated fresh-code recovery screen, and a profile trigger prevents a modified client from bypassing the gate. Preview, ordinary redemption, signup, and recovery share the same reservation-aware capacity rule. The local seed explicitly selects `development_open`; hosted remains `invitation_required`.
...
- Hosted development has all nine migrations with matching history and 285 passing remote assertions. The Before User Created hook is enabled only for `pg-functions://postgres/public/before_user_created_invitation_gate`; a real code-free hosted Auth request returns the intended generic 403 and creates zero Auth rows. Both hosted advisors report no issues. Older pgTAP files now select transaction-only `development_open` explicitly for direct Auth fixtures, so they run identically with or without the local seed and roll the mode change back.
...
Hosted linking and all nine current migrations, CI/repository safeguards, the simulator-based Phase 2 implementation, the Circle lifecycle, and invitation-gated signup deployment are complete. All 285 hosted assertions and both advisors are green, and the real hosted Auth hook denies code-free signup without leaving a user row. The remaining Phase 3 gate is simulator visual acceptance of signup, verification, fresh-code recovery, and overlapping-Circle administration before beginning Phase 4. Hosted SMTP/bot protection, final legal copy, and physical/platform acceptance remain explicit pre-external-testing gates.
...
Run the short simulator acceptance pass for invitation-code signup, email verification, fresh-code recovery, and three-user overlapping-Circle administration. Record any visual defects, then close the Phase 3 gate and begin Phase 4's private one-photo posting vertical slice. Hosted Auth/SMTP/bot-protection review remains a separate pre-external-tester gate rather than a blocker for simulator development.

Starter prompt:

> Read `AGENTS.md` and `PROJECT.md`, inspect the repository, and continue the documented Phase 3 simulator acceptance checkpoint. If it passes, update the milestone and begin the Phase 4 private-photo vertical slice.
