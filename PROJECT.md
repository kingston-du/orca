...
Implementation state: Phases 1 through 4 complete and promoted; Checkpoint 5A complete locally and awaiting hosted promotion. Hosted development runs the promoted eight-migration history and carries both media buckets, all three Edge Functions, both Vault secrets, and both Cron schedules, all verified against real HTTP. Local adds one unpromoted migration, `20260802120000_recent_feed.sql`.
...
### Checkpoint 5A — Real card/deck and gesture acceptance (**complete locally; hosted promotion and physical-device acceptance pending**)
...
Phases 1 through 4 are complete and promoted, and Checkpoint 5A is implemented and green on every local gate. Hosted development carries the promoted eight-migration history, both media buckets, all three Edge Functions, both Vault secrets, `pg_cron`/`pg_net`, and both Cron schedules, all verified against real HTTP.

The one divergence is deliberate: `20260802120000_recent_feed.sql` exists locally and has not been pushed.

The next action is **promoting Checkpoint 5A to hosted development**. It adds one read RPC and replaces one unused index; it creates no bucket, deploys no function, and changes no data. It still needs the founder's explicit promotion approval, because every hosted migration push does.

Hosted promotion approval needed:

> Approve Codex to promote Checkpoint 5A's `20260802120000_recent_feed.sql` to hosted development and re-run the real suites against it.

After that, the next implementation action is **Checkpoint 5B — Home sessions and history surfaces**, exactly as scoped in Section 27. It has not been authorized.
...
- **Physical-iPhone publish acceptance (from Phase 4).** Real native upload progress, cancel mid-transfer, a backgrounded transfer that outlives the foreground, process death between upload and finalize, publishing on named Wi-Fi and LTE profiles, the media memory ceiling from Section 17, and a VoiceOver/Dynamic Type audit of the composer's publish, progress, review, and unknown-outcome states. The simulator cannot exercise the camera or a real background URLSession, so nothing about this gate can be closed off-device.
