...
- Both Circle lifecycle migrations were promoted only after the linked ref, ignored app URL, project name, region, health, and exact two-file dry run matched `orca-dev` (`evuqnmvcnqhkzkitszqp`, `ca-central-1`). All eight hosted histories match, 247/247 hosted assertions pass, and hosted Security and Performance Advisors have no warnings or errors. Advisor INFO entries are limited to RLS-without-policy on intentionally unexposed private tables and one fresh unused legal index.
...
- Phase 4 now has its embedded-camera and normalization foundation. Expo Camera owns a full-screen photo-only preview in the Camera tab with rear/front switching, autofocus, orientation-aware capture, mirrored selfies, Auto/Off flash, a guarded shutter, and camera lifecycle tied to tab focus plus foreground state. The system picker remains the narrow existing-photo path without broad library permission. Both sources are re-encoded through one local JPEG contract: 2048-pixel maximum long edge, `0.82` quality, measured 6 MiB maximum, explicit capture instant/UTC offset/source, and no retained picker EXIF. Native configuration removes microphone, barcode, audio, and broad Android media/storage/location capabilities. The app does not upload or persist a selected photo yet; physical iPhone output/orientation/metadata evidence and a rebuilt development client remain pending.
- Phase 4's local posting backend foundation is implemented but deliberately not promoted. Migration `20260731005750` adds constrained pending/published/deleting posts, immutable canonical `{circle_id}/{author_id}/{post_id}/media.jpg` paths, a 24-hour idempotent reserve contract, forward-only lifecycle protection, current-member plus author-fallback visibility, historical profile attribution, and a private JPEG-only 6 MiB `post-media` bucket. Migration `20260731023409` plus authenticated `finalize-post` add bounded server-side JPEG decoding, service-only measured evidence, current-authorization rechecks, atomic publication, and canonical retries without trusting client media claims. Migration `20260731044546` plus authenticated `delete-post` and secret-only `reconcile-post-media` add the private retryable cleanup outbox, author cancel/delete and former-author fallback, expiring leases/backoff, and expired-pending/orphan discovery. Migration `20260731050907` plus authenticated `delete-circle` extend that outbox to Circle-wide cleanup: the request freezes/hides the Circle, revokes invites, marks every post deleting, enqueues post and Circle-prefixed orphan paths, removes bytes in bounded leased Storage-API batches, and deletes relational state only after posts, child jobs, and Storage metadata are all empty. A private receipt survives completion for nonrevealing lost-response retries; sole-member account preparation uses the same media-safe primitive. Database SQL only reads Storage metadata and never deletes `storage.objects`. Clean 14-migration replay, database lint/types, all 420 pgTAP assertions, 12 pure verifier/orchestration tests, the real Storage authorization suite, and real finalization plus post/Circle cleanup HTTP suites pass. Hosted Vault/Cron activation, the app upload/finalize path, physical-iPhone acceptance, and hosted promotion remain pending.
- `docs/course/` records each completed checkpoint as a self-contained tutorial covering the mental model, runtime flow, important code and syntax, state/security ownership, focused tests, and verification evidence. Lessons 6–15 expand the format with end-to-end stories and diagrams, lock/concurrency reasoning, real debugging findings, concise glossaries, and review checklists; Lesson 7 is clearly marked as superseded architecture, Lesson 8 teaches the current open-account/invite-only-Circle boundary, Lesson 9 closes Phase 3, Lessons 10–11 teach the native media pipeline, Lesson 12 teaches pending reservations plus exact private-Storage authorization, Lesson 13 teaches trusted byte inspection plus atomic publication, Lesson 14 teaches the child outbox/lease/reconciliation model, and Lesson 15 teaches parent/child Circle cleanup across database and private Storage.
...
- Hosted development still has exactly the first ten migrations with matching history and 259 passing remote assertions. Local migrations `20260731005750`, `20260731023409`, `20260731044546`, and `20260731050907`, plus the `finalize-post`, `delete-post`, `delete-circle`, and `reconcile-post-media` Edge Functions, are intentionally unpromoted/undeployed. The obsolete signup hook and signup-gate database objects are absent; Security and Performance Advisors report no warning/error findings.
...
- No hosted private Storage buckets or production Supabase project; `post-media` exists only in the rebuilt local stack
...
- Opening the tab presents the embedded Expo Camera as the primary low-friction capture surface; Choose Photo remains a secondary system-picker action.
...
- [x] Add versioned `legal_acceptances` plus a narrow server-timestamped acceptance RPC that allows only current document versions/hashes; require adult eligibility and current acceptance before protected creation/interaction operations.
...
- [x] Enable local email confirmation and configure the version-controlled in-app verification-code email template. Promote equivalent hosted behavior only with reviewed Auth settings and custom SMTP.
- [x] Use local Mailpit in development and verify a real signup email/code/session exchange.
...
- [x] Add `jest-expo` and React Native Testing Library at current Expo-compatible versions before the first Auth component tests; grow focused tests with each phase.
- [x] Add one app-level TanStack Query provider/client for profile and later remote rows; keep Auth session state outside it and clear the client on identity change.
...
- [x] Add the narrow validated roster RPC that returns only member ID, display name, and role even when full profiles are blocked later.
- [x] Add transactional invite creation, preview/redemption, expiry, usage, and revocation.
...
- [x] Keep account creation open so any organizer can start a new Circle. Require bounded, expiring, revocable invitations only when joining an existing Circle; account verification and onboarding never depend on Circle admission.
...
- [x] Add `posts` with pending/published/deleting lifecycle, constraints, indexes, grants, RLS, and tests locally; hosted promotion remains gated.
- [x] Create the local private `post-media` bucket with MIME/size restrictions; hosted creation follows migration promotion.
...
- [x] Add the idempotent reserve operation with caller/membership/path validation.
- [x] Add trusted finalize with caller/pending-object/verified-media validation locally; hosted promotion remains gated.
- [x] Make finalization a trusted authenticated Edge Function that derives the caller, validates the exact pending object, verifies JPEG magic/decodability and actual byte size/dimensions, then calls a private transactional publish helper. Client-supplied dimensions/MIME are never security inputs. Local unit, pgTAP, and real HTTP integration suites pass; deployment remains gated.
- [x] Require an exact unexpired pending row in upload policy and matching published row in read policy.
- [x] Test real Storage API behavior in addition to SQL policy tests, including upload `RETURNING`, MIME/size limits, wrong paths, upsert denial, and private reads.
- [x] Add the `object.upload`-scoped pending-author Storage SELECT needed by upload `INSERT … RETURNING`; ordinary pending downloads and nonauthors remain denied.
- [x] Expand profile visibility to current viewers of a still-visible historical contribution, and add author-only post/media read fallback after leaving without exposing the old feed.
- [x] Add the author-only controlled delete fallback as part of the retryable cleanup workflow locally; hosted deployment remains gated.
- [x] Implement author cancel/delete: hide/mark deleting → remove object through Storage API → remove metadata, with retryable private job state. Lost responses, live/expired leases, missing objects, partial failure, concurrency, and former-author access have local SQL/unit/real-HTTP coverage.
...
- [x] Implement stale-pending/orphan reconciliation through the trusted cleanup outbox and secret-only Edge worker locally; PostgreSQL reads Storage metadata for discovery but all object removal uses the Storage API.
- [ ] At hosted deployment, create a named automation secret, store the project URL/key in Vault, schedule bounded reconciliation with Supabase Cron, and verify job history plus an injected retry before alpha. Do not commit a fake URL/key or activate a schedule before the function exists remotely.
...
- [ ] Keep account signup open while adding sensible per-user/IP cost-abuse limits for Auth email, invite generation/redemption, uploads, comments, and reports; test ordinary friend use is not blocked.
...
Implement the next local-only Phase 4 checkpoint by wiring the normalized camera/picker result into the one-photo composer and trusted publish flow: choose one current Circle, preview the exact normalized JPEG and capture time, optionally add a caption, reserve the canonical post/path, upload without upsert, call `finalize-post`, and expose clear progress, cancellation, retry, and recoverable failure states. Keep route files thin, keep the selected file/draft out of global server state, invalidate only the relevant feed queries, and use `delete-post` to clean an abandoned reservation after upload may have begun. Add focused component/API tests and preserve exact server authorization as the final boundary. Do not promote the four posting migrations or deploy functions yet. Hosted Vault/Cron activation, physical-iPhone media acceptance, and the rebuilt media-enabled development client remain subsequent gates.

Starter prompt:

> Read `AGENTS.md` and `PROJECT.md`, inspect the repository, and continue the documented local-only Phase 4 one-photo app upload/finalize checkpoint. Do not promote migrations or deploy functions; retain the physical-iPhone media acceptance gate.
