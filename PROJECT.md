...
- Phase 4's local posting authorization foundation is implemented but deliberately not promoted. Migration `20260731005750` adds constrained pending/published/deleting posts, immutable canonical `{circle_id}/{author_id}/{post_id}/media.jpg` paths, a 24-hour idempotent reserve contract, forward-only lifecycle protection, current-member plus author-fallback visibility, historical profile attribution, and a private JPEG-only 6 MiB `post-media` bucket. Storage INSERT requires the exact unexpired pending row; its narrow pending SELECT is scoped to `object.upload`, while normal pending download, nonauthor/wrong-path writes, upsert, and client delete remain denied. All 312 pgTAP assertions and the real local Storage HTTP suite pass, including MIME/size enforcement and published member/former-author reads. Actual JPEG verification/finalization, cleanup workflows, app upload state, physical iPhone acceptance, and hosted promotion remain pending.
- `docs/course/` records each completed checkpoint as a self-contained tutorial covering the mental model, runtime flow, important code and syntax, state/security ownership, focused tests, and verification evidence. Lessons 6–12 expand the format with end-to-end stories and diagrams, lock/concurrency reasoning, real debugging findings, concise glossaries, and review checklists; Lesson 7 is clearly marked as superseded architecture, Lesson 8 teaches the current open-account/invite-only-Circle boundary, Lesson 9 closes Phase 3, Lessons 10–11 teach the native media pipeline, and Lesson 12 teaches pending reservations plus exact private-Storage authorization.
...
- [ ] Add trusted finalize with caller/pending-object/verified-media validation.
...
Implement the local-only trusted publish-finalization checkpoint: authenticate the caller in an Edge Function, load and inspect the exact pending `post-media` object, verify JPEG magic/decodability and actual byte size/dimensions, then call a narrow transactional helper that atomically sets canonical media facts and database sharing time. Add real negative tests for forged bytes, dimensions, paths, membership, replay, and missing objects. Do not promote migrations or deploy the function yet. Physical-iPhone output/orientation/metadata acceptance remains required before the upload/finalize flow is promoted or called complete.

Starter prompt:

> Read `AGENTS.md` and `PROJECT.md`, inspect the repository, and continue the documented local-only Phase 4 trusted publish-finalization checkpoint. Do not promote migrations or deploy functions; retain the physical-iPhone media acceptance gate.
