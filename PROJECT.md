...
- Local-only migration `20260729072231` implements invitation-gated signup. Auth receives a validated raw invite only as transient metadata; a `BEFORE` trigger strips it before storage and leaves a server-derived marker long enough for the authoritative Auth insert trigger to reserve hash-backed capacity. Email confirmation claims membership, invalidated reservations route to an authenticated fresh-code recovery screen, and a profile trigger prevents a modified client from bypassing the gate. Preview, ordinary redemption, signup, and recovery share the same reservation-aware capacity rule. The local seed explicitly selects `development_open`; the migration default for hosted/production is `invitation_required`.
- A real local Auth/Mailpit flow proved that code-free signup creates no Auth row, invited signup stores neither the raw token nor its internal marker, membership waits for verification, and OTP confirmation claims exactly one membership/use. A clean nine-migration reset, all 285 pgTAP assertions, zero-warning database lint/advisors, exact generated types, all 56 app tests, formatting, TypeScript, zero-warning lint, Expo compatibility, Expo Doctor 20/20, legal fingerprints, and an iOS export bundle pass.
- `docs/course/` records each completed checkpoint as a self-contained tutorial covering the mental model, runtime flow, important code and syntax, state/security ownership, focused tests, and verification evidence. Lessons 6–7 expand the format with end-to-end stories and diagrams, lock/concurrency reasoning, real debugging findings, concise glossaries, and review checklists; future coherent checkpoints must remain detailed enough to learn from without first reverse-engineering every implementation file.
...
- Hosted development has eight migrations with matching history, 247 passing remote assertions, and no hosted advisor warnings/errors. A fresh exact-target check confirms the linked project and app URL both reference healthy `orca-dev`; the dry run names only the local ninth invitation-gated-signup migration and applies nothing. That migration still needs explicit promotion approval. The version-controlled Auth-hook entry also needs corresponding hosted Auth-hook configuration; the database trigger remains the non-bypassable final gate if hook configuration drifts.
- Phase 1B is complete. Do not amend any migration already recorded remotely.
- TypeScript, formatting, zero-warning lint, dependency compatibility, all 56 app tests, an iOS export bundle, and all 20 `expo-doctor` checks pass locally. The protected route boundary and original Auth entry form have been accepted in Expo Go; the invitation-code signup/recovery addition plus verification, recovery, onboarding, Settings/sign-out, and Circle screens still need quick visual passes after entering their flows.
...
Hosted linking and the first eight migrations, CI/repository safeguards, the simulator-based Phase 2 implementation, and the Circle lifecycle promotion are complete. Invitation-gated signup is implemented and fully verified locally as the ninth migration. Exact-target verification and a dry run naming only `20260729072231_add_invitation_gated_signup.sql` pass without applying it. The next separate gate is explicit promotion approval, followed by hosted assertions/advisors and hosted Auth-hook configuration. Hosted SMTP/bot protection, final legal copy, visual passes, and physical/platform acceptance remain explicit pre-external-testing gates.
...
- [x] Carry a validated invite through sign-up/verification and redeem it only after the verified account exists. Enforce the gate at both the supported Auth hook and an authoritative database trigger so a custom client cannot bypass it by calling `signUp` directly. Production/hosted defaults to invitation-required; only the local seed enables the explicitly environment-bound bootstrap mode. Hosted promotion and Auth-hook configuration remain the immediate deployment gate.
...
With explicit approval, apply the already reviewed dry run's only pending change, `20260729072231_add_invitation_gated_signup.sql`, to verified development project `orca-dev` (`evuqnmvcnqhkzkitszqp`). Then configure the hosted Before User Created Auth hook to `pg-functions://postgres/public/before_user_created_invitation_gate`, run all hosted assertions and both advisors, and visually accept signup, verification, fresh-code recovery, and Circle flows before closing Phase 3.

Starter prompt:

> Read `AGENTS.md` and `PROJECT.md`, inspect the repository, and continue the documented invitation-gated-signup hosted deployment gate. The exact `orca-dev` target and sole pending migration are already dry-run; apply only after explicit approval, then verify remotely and configure the Auth hook.
