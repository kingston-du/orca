...
- Phase 4's local posting backend foundation is implemented but deliberately not promoted. Migration `20260731005750` adds constrained pending/published/deleting posts, immutable canonical `{circle_id}/{author_id}/{post_id}/media.jpg` paths, a 24-hour idempotent reserve contract, forward-only lifecycle protection, current-member plus author-fallback visibility, historical profile attribution, and a private JPEG-only 6 MiB `post-media` bucket. Storage INSERT requires the exact unexpired pending row; its narrow pending SELECT is scoped to `object.upload`, while normal pending download, nonauthor/wrong-path writes, upsert, and client delete remain denied. Migration `20260731023409` adds service-only short-lived verification evidence and a narrow user-scoped transactional finalizer. The authenticated `finalize-post` Edge Function derives the Auth caller, loads the RLS-visible reservation, downloads its exact private object with the server client, enforces 6-MiB/2048-pixel preflight limits, strictly decodes JPEG bytes within explicit memory/resolution bounds, records measured facts, and atomically publishes with database sharing time. Exact and concurrent retries return one canonical post; client-supplied paths/MIME/dimensions are never trusted. Clean local replay, database lint/types, all 350 pgTAP assertions, five pure verifier tests, the real Storage HTTP suite, and the real Auth→Function→Storage→database HTTP suite pass. Cleanup workflows, the app upload path, physical-iPhone acceptance, and hosted promotion remain pending.
- `docs/course/` records each completed checkpoint as a self-contained tutorial covering the mental model, runtime flow, important code and syntax, state/security ownership, focused tests, and verification evidence. Lessons 6–13 expand the format with end-to-end stories and diagrams, lock/concurrency reasoning, real debugging findings, concise glossaries, and review checklists; Lesson 7 is clearly marked as superseded architecture, Lesson 8 teaches the current open-account/invite-only-Circle boundary, Lesson 9 closes Phase 3, Lessons 10–11 teach the native media pipeline, Lesson 12 teaches pending reservations plus exact private-Storage authorization, and Lesson 13 teaches trusted byte inspection plus atomic publication.
...
- Hosted development still has exactly the first ten migrations with matching history and 259 passing remote assertions. Local migrations `20260731005750` and `20260731023409`, plus the `finalize-post` Edge Function, are intentionally unpromoted/undeployed. The obsolete signup hook and signup-gate database objects are absent; Security and Performance Advisors report no warning/error findings.
...
- [ ] Add the author-only controlled delete fallback as part of the retryable cleanup workflow.
...
Implement the next local-only trusted media-cleanup checkpoint: add author cancel/delete state transitions and a retryable server workflow that removes private objects through the Storage API before relational completion, then reconcile expired pending rows and orphaned `post-media` objects without deleting Storage metadata directly in SQL. Cover lost responses, missing objects, partial failure, former-author fallback, and concurrent retries. Do not promote either posting migration or deploy functions yet. Circle-wide deletion cleanup, app upload/finalize wiring, and physical-iPhone media acceptance remain subsequent gates.

Starter prompt:

> Read `AGENTS.md` and `PROJECT.md`, inspect the repository, and continue the documented local-only Phase 4 trusted media-cleanup checkpoint. Do not promote migrations or deploy functions; retain the physical-iPhone media acceptance gate.
