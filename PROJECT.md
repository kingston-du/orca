...
- Phase 4's local posting backend foundation is implemented but deliberately not promoted. Migration `20260731005750` adds constrained pending/published/deleting posts, immutable canonical `{circle_id}/{author_id}/{post_id}/media.jpg` paths, a 24-hour idempotent reserve contract, forward-only lifecycle protection, current-member plus author-fallback visibility, historical profile attribution, and a private JPEG-only 6 MiB `post-media` bucket. Migration `20260731023409` plus the authenticated `finalize-post` Edge Function add bounded server-side JPEG decoding, service-only measured evidence, current-authorization rechecks, atomic publication, and canonical retries without trusting client media claims. Migration `20260731044546` adds a private retryable cleanup outbox, author cancel/delete and former-author fallback, expiring worker leases, bounded backoff, expired-pending/orphan discovery, and service-only claim/complete/fail RPCs. The authenticated `delete-post` function hides first, removes the immutable object through the Storage API, and only then completes relational deletion; secret-only `reconcile-post-media` uses the same worker for drift. Database SQL only reads Storage metadata for orphan discovery and never deletes `storage.objects`. Clean local replay, database lint/types, all 389 pgTAP assertions, eight pure verifier/orchestration tests, the real Storage authorization suite, and real finalization plus cleanup HTTP suites pass. Circle-wide cleanup, the hosted Vault/Cron activation, the app upload path, physical-iPhone acceptance, and hosted promotion remain pending.
- `docs/course/` records each completed checkpoint as a self-contained tutorial covering the mental model, runtime flow, important code and syntax, state/security ownership, focused tests, and verification evidence. Lessons 6–14 expand the format with end-to-end stories and diagrams, lock/concurrency reasoning, real debugging findings, concise glossaries, and review checklists; Lesson 7 is clearly marked as superseded architecture, Lesson 8 teaches the current open-account/invite-only-Circle boundary, Lesson 9 closes Phase 3, Lessons 10–11 teach the native media pipeline, Lesson 12 teaches pending reservations plus exact private-Storage authorization, Lesson 13 teaches trusted byte inspection plus atomic publication, and Lesson 14 teaches the outbox/lease/reconciliation cleanup model.
...
- Hosted development still has exactly the first ten migrations with matching history and 259 passing remote assertions. Local migrations `20260731005750`, `20260731023409`, and `20260731044546`, plus the `finalize-post`, `delete-post`, and `reconcile-post-media` Edge Functions, are intentionally unpromoted/undeployed. The obsolete signup hook and signup-gate database objects are absent; Security and Performance Advisors report no warning/error findings.
...
Implement the next local-only Phase 4 checkpoint by extending the trusted cleanup outbox to retryable Circle deletion: freeze and hide the Circle, enqueue every post/media path, remove all objects through the Storage API across bounded leased batches, and delete Circle relational state only after no media work remains. Preserve deterministic admin authorization, nonrevealing retries, immediate ordinary-access denial, partial-failure recovery, and established lock order. Do not promote the three posting migrations or deploy functions yet. Hosted Vault/Cron activation, app upload/finalize wiring, and physical-iPhone media acceptance remain subsequent gates.

Starter prompt:

> Read `AGENTS.md` and `PROJECT.md`, inspect the repository, and continue the documented local-only Phase 4 retryable Circle-deletion cleanup checkpoint. Do not promote migrations or deploy functions; retain the physical-iPhone media acceptance gate.
