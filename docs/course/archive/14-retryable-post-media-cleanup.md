...
> **Archived Circle-era lesson.** The hide-first outbox, leases, bounded backoff, and Storage API cleanup pattern remains directly reusable, but its row authorization and former-Circle access assumptions are obsolete. Reapply the mechanism to the approved friend-first Moment model. See [`PROJECT.md`](../../../PROJECT.md).
