...
> **Archived and superseded — do not implement.** Orca permanently restored open account signup, and the friend-first redesign does not revive a signup gate. This remains only a study of triggers, bearer-token handling, and transactional reservations. See [`PROJECT.md`](../../../PROJECT.md).

> **Superseded architecture (July 29, 2026):** This lesson records a working design that Orca intentionally removed. At the time, [Lesson 8](./08-open-accounts-and-invite-only-circles.md) superseded it by restoring open accounts while keeping invite-only Circles. Both Circle flows are now archived. This lesson remains useful for learning triggers and transactional reservations, but its described product flow is not active Orca behavior.

## Outcome

At this historical checkpoint, Orca’s signup path was invitation-gated in the **local database and app**. A new person had to supply a private invitation code, confirm their email, and hold a claimed admission before completing onboarding. The code was a short-lived bearer capability: it entered Auth briefly, was converted into a hash-backed server record, and was scrubbed before the Auth user transaction committed.
...
At commit `c138432`, the now-removed `supabase/tests/invitation_gated_signup_test.sql` contained **38 pgTAP assertions**, bringing both clean local and hosted suites to **285 passing assertions**. It covered function grants/RLS, hook rejection, raw-token scrubbing, account/profile creation, reservation capacity, second-signup rejection without a partial Auth row, confirmation claim, revocation recovery, onboarding denial until a claim, and `development_open` behavior. Older fixture files chose transaction-only bootstrap mode explicitly instead of depending on the local seed; the mode change rolled back with each test. Commit `db6dd3a` removed that obsolete test with the signup gate.
