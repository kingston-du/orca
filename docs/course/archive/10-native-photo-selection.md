...
> **Archived and superseded — do not implement as written.** Lesson 11 replaced the system-camera surface with the embedded camera and absorbed the still-current scoped-picker boundary. Two source files referenced below were removed during that replacement. See [`Lesson 11`](../11-embedded-camera-and-image-normalization.md) and [`PROJECT.md`](../../../PROJECT.md).
...
The route [`camera.tsx`](<../../../src/app/(app)/(tabs)/camera.tsx>) was intentionally thin. At commit `26dce7e`, UI and behavior lived in the now-removed `src/features/posts/photo-source-picker-screen.tsx`, where the feature could be tested without a router. Commit `0bea5ae` replaced that screen with the embedded `PhotoCaptureScreen` taught in Lesson 11.
...
At commit `26dce7e`, the now-removed `src/features/posts/__tests__/photo-source-picker-screen.test.tsx` proved the user-facing connection:
