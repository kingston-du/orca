...
> **Archived Circle-era lesson.** Trusted byte measurement, exact-object lookup, and atomic publication remain sound, but final authorization and publication facts must be rebuilt for friend recipients, tags, and recent-versus-archive behavior. Do not implement its Circle-bound transaction unchanged. See [`PROJECT.md`](../../../PROJECT.md).
